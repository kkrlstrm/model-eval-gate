...
import { fieldAgreementGrader, recordedHumanGrader, llmJudgeGrader, type Grader } from './graders.ts';
...
function buildGrader(cfg: any): Grader {
  switch (cfg.kind) {
    case 'fieldAgreement': return fieldAgreementGrader(cfg.name ?? 'agreement', cfg.fields);
    case 'human': return recordedHumanGrader(cfg.name ?? 'human');
    case 'llmJudge': return llmJudgeGrader(cfg.name ?? 'judge', cfg.model, cfg.rubric);
    default: throw new Error(`unknown grader kind: ${cfg.kind}`);
  }
}
...
for (const specPath of specFiles) {
  ...
  console.log(`   model=${modelSpec.model}  k=${k}  tasks=${tasks.length}${modelSpec.provider ? `  pin=${JSON.stringify(modelSpec.provider)}` : ''}`);

  const suite = await runSuite({
    client, spec: modelSpec, tasks, parse: parseJsonObject, graders, k,
  ...
  if (base.model && base.model !== modelSpec.model) {
    drift.push(`MODEL SWAP: baseline on ${base.model}, allowlist now routes to ${modelSpec.model}`);
  }
  if (base.meanScore != null && suite.meanScore < base.meanScore - (spec.drift_tolerance ?? 0.1)) {
    drift.push(`SCORE DRIFT: meanScore ${suite.meanScore.toFixed(3)} < baseline ${base.meanScore} − tol ${spec.drift_tolerance ?? 0.1}`);
  }
  if (spec.consistency_floor != null && suite.passHatK < spec.consistency_floor) {
    drift.push(`CONSISTENCY: pass^k ${suite.passHatK.toFixed(2)} < floor ${spec.consistency_floor}`);
  }

  console.log(`   pass@k=${suite.passAtK.toFixed(2)}  pass^k=${suite.passHatK.toFixed(2)}  meanScore=${suite.meanScore.toFixed(3)}  cost=$${suite.totalCost.toFixed(4)}  ${(suite.totalMs / 1000).toFixed(0)}s`);
  console.log(`   providers served: ${suite.providersSeen.length ? suite.providersSeen.join(', ') : '(unreported)'}`);
  if (base.meanScore != null) console.log(`   baseline: meanScore=${base.meanScore}  pass^k=${base.passHatK ?? '—'}  on ${base.model ?? '—'} (${base.verified_date ?? 'unrecorded'})`);
  ...
  // ── bootstrap / refresh the baseline on request ──
  if (updateBaseline) {
    spec.baseline = {
  ...
  const weakTasks = suite.tasks
    ...
    .map((t) => {
      ...
      const misses = (worst?.graders ?? [])
        .flatMap((g) => g.assertions.filter((a) => !a.pass).map((a) => `${a.name}: ${a.detail ?? 'miss'}`));
      return { id: t.task_id, meanScore: Number(t.meanScore.toFixed(3)), passHatK: t.passHatK, misses };
    });
  allResults.push({
    mode: spec.mode, model: modelSpec.model,
    passAtK: suite.passAtK, passHatK: suite.passHatK, meanScore: suite.meanScore,
    totalCost: suite.totalCost, providersSeen: suite.providersSeen,
    baseline: spec.baseline ?? {}, drift, weakTasks,
  });
}

const outPath = join(HERE, 'regression-results.json');
writeFileSync(outPath, JSON.stringify({ ran_at: new Date().toISOString(), results: allResults }, null, 2) + '\n');
console.log(`\nwrote ${outPath}`);
console.log(anyDrift ? '\n✗ regression: DRIFT detected (see above).' : '\n✓ regression: all specs within baseline.');
